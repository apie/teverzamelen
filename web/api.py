...
def api_page():
    ...
    return render_template(
        "api.html", title="API overzicht", api_key=getattr(api_key, "key", None)
    )
...
def api_reading_list(key):
    ...
    return dict(
        ...
        num_to_read=Item.query.filter_by(owned=True, read=False)
        .join(Item.collection)
        .filter_by(user=user)
        .count(),
        num_busy_reading=Item.query.join(Item.collection)
        .filter_by(user=user)
